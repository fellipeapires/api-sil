...
public interface DistribuicaoRepository extends JpaRepository<Distribuicao, Long>, DistribuicaoRepositoryQuery {
	...
	@Modifying
	@Transactional
	@Query(value = "INSERT INTO MED_DISTRIBUICAO_LEITURA_REGISTRO (ID_LEITURA, ID_REGIONAL, ID_USUARIO, FL_ASSOCIADO, DT_DISTRIBUICAO, CD_GRUPO_FATURAMENTO, DT_ANO_MES_REF) "
			+ "SELECT A.ID_LEITURA, ?1, ?4, ?8, ?9, ?3, ?2 " 
	...
	@Transactional
	@Query(value = "INSERT INTO MED_DISTRIBUICAO_LEITURA_REGISTRO (ID_LEITURA, ID_REGIONAL, ID_USUARIO, FL_ASSOCIADO, DT_DISTRIBUICAO, CD_GRUPO_FATURAMENTO, DT_ANO_MES_REF) "
			+ "SELECT A.ID_LEITURA, ?1, ?5, ?7, ?8, ?3, ?2 "
			+ "FROM MED_LEITURA AS A "
			+ "INNER JOIN MED_IMPORTACAO AS B ON A.ID_IMPORTACAO = B.ID_IMPORTACAO "
			+ "INNER JOIN MED_GRUPO_FATURAMENTO AS D ON D.ID_GRUPO_FATURAMENTO = B.ID_GRUPO_FATURAMENTO "
			+ "INNER JOIN MED_DISTRIBUICAO_LEITURA_REGISTRO AS E ON E.ID_LEITURA = A.ID_LEITURA AND E.DT_ANO_MES_REF = ?2 AND E.ID_REGIONAL = ?1 AND E.CD_GRUPO_FATURAMENTO = ?3 "
			+ "LEFT OUTER JOIN MED_RETORNO_LEITURA AS C ON C.ID_LEITURA = A.ID_LEITURA AND C.ID_REGIONAL = ?1 AND C.FL_ATIVO = 1 "
	...
	@Query(value="SELECT D.ID_USUARIO AS IDUSUARIO, A.ID_LEITURA AS IDLEITURA "
			...
			+ "LEFT OUTER JOIN MED_RETORNO_LEITURA AS E ON E.ID_LEITURA = D.ID_LEITURA AND E.ID_REGIONAL = ?1 AND E.FL_ATIVO = 1 "
	...
	@Query(value="SELECT A.ID_USUARIO AS IDUSUARIO, A.ID_LEITURA AS IDLEITURA "
			...
			+ "LEFT OUTER JOIN MED_RETORNO_LEITURA AS B ON B.ID_LEITURA = A.ID_LEITURA AND B.FL_ATIVO = 1 "
	...
	@Query(value="DELETE "
			...
			+ "		LEFT OUTER JOIN MED_RETORNO_LEITURA AS E ON E.ID_LEITURA = A.ID_LEITURA AND E.ID_REGIONAL = ?2 AND E.FL_ATIVO = 1 "
			+ "WHERE "
	...
	@Query(value="SELECT B.ID_REGIONAL AS IDREGIONAL, D.CD_GRUPO_FATURAMENTO AS GRUPOFATURAMENTO, B.DT_ANO_MES_REF AS DATAREFERENCIA, "
			...
			+ "INNER JOIN MED_IMPORTACAO AS B ON A.ID_IMPORTACAO = B.ID_IMPORTACAO "
			+ "INNER JOIN MED_GRUPO_FATURAMENTO AS D ON D.ID_GRUPO_FATURAMENTO = B.ID_GRUPO_FATURAMENTO "
			+ "INNER JOIN MED_DISTRIBUICAO_LEITURA_REGISTRO AS C ON C.ID_LEITURA = A.ID_LEITURA AND C.DT_ANO_MES_REF = ?1 AND C.ID_REGIONAL = ?2 AND C.CD_GRUPO_FATURAMENTO = ?3 "
	...
	@Query(value="SELECT "
			...
			+ "	LEFT OUTER JOIN MED_RETORNO_LEITURA AS E ON E.ID_LEITURA = A.ID_LEITURA AND E.FL_ATIVO = 1 AND E.ID_REGIONAL = ?1 AND E.FL_ATIVO = 1 "
			+ "WHERE "
			+ "	B.ID_REGIONAL = ?1 "
	...
	@Query(value = "SELECT 0 AS IDLEITURA, B.ID_REGIONAL AS IDREGIONAL, D.CD_GRUPO_FATURAMENTO AS GRUPOFATURAMENTO, B.DT_ANO_MES_REF AS DATAREFERENCIA, "
			...
			+ "INNER JOIN MED_IMPORTACAO AS B ON A.ID_IMPORTACAO = B.ID_IMPORTACAO "
			+ "INNER JOIN MED_GRUPO_FATURAMENTO AS D ON D.ID_GRUPO_FATURAMENTO = B.ID_GRUPO_FATURAMENTO "
			+ "LEFT OUTER JOIN MED_DISTRIBUICAO_LEITURA_REGISTRO AS C ON C.ID_LEITURA = A.ID_LEITURA AND C.DT_ANO_MES_REF = ?1 AND C.ID_REGIONAL = ?2 AND C.CD_GRUPO_FATURAMENTO = ?3 "
			+ "LEFT OUTER JOIN MED_RETORNO_LEITURA AS E ON E.ID_LEITURA = A.ID_LEITURA AND E.ID_REGIONAL = ?2 AND E.FL_ATIVO = 1 "
			+ "WHERE B.DT_ANO_MES_REF = ?1 AND B.ID_REGIONAL = ?2 AND D.CD_GRUPO_FATURAMENTO = ?3 "
	...
	@Modifying
	@Transactional
	@Query(value="UPDATE MED_DISTRIBUICAO_LEITURA_REGISTRO SET FL_ASSOCIADO = 1 where ID_DISTRIBUICAO_LEITURA_REGISTRO IN("
			+ "SELECT C.ID_DISTRIBUICAO_LEITURA_REGISTRO "
			+ "FROM MED_LEITURA AS A "
			+ "INNER JOIN MED_IMPORTACAO AS B ON A.ID_IMPORTACAO = B.ID_IMPORTACAO "
			+ "INNER JOIN MED_GRUPO_FATURAMENTO AS D ON D.ID_GRUPO_FATURAMENTO = B.ID_GRUPO_FATURAMENTO "
			+ "INNER JOIN MED_DISTRIBUICAO_LEITURA_REGISTRO AS C ON C.ID_LEITURA = A.ID_LEITURA "
			+ "INNER JOIN MED_RETORNO_LEITURA AS E ON E.ID_LEITURA = A.ID_LEITURA "
			+ "WHERE B.DT_ANO_MES_REF = ?1 AND B.ID_REGIONAL = ?2 AND C.FL_ASSOCIADO = 2 "
			+ ")  ", nativeQuery = true)
	public void marcarNoColetor(LocalDate dataReferencia, Long idRegional);
