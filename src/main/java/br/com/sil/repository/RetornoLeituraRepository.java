...
public interface RetornoLeituraRepository extends JpaRepository<RetornoLeitura, Long>, RetornoLeituraRepositoryQuery {
	...
	@Query(value = "SELECT B.ID_REGIONAL AS IDREGIONAL, B.DT_ANO_MES_REF AS DATAREFERENCIA, "
			...
			+ "INNER JOIN SEG_USUARIO AS E2 ON E2.ID_USUARIO = E.ID_USUARIO "
			+ "INNER JOIN MED_RETORNO_LEITURA AS F2 ON F2.ID_LEITURA = A2.ID_LEITURA AND F2.ID_REGIONAL = ?1 AND F2.FL_ATIVO = 1 "
			+ "WHERE B2.ID_REGIONAL = ?1 AND B2.DT_ANO_MES_REF = ?2 AND C2.CD_GRUPO_FATURAMENTO = ?3 "
			+ "AND F2.ID_USUARIO = E.ID_USUARIO AND A2.CD_TAREFA_LEITURA = A.CD_TAREFA_LEITURA) "
			+ "AS DATAINICIO, "
			...
			+ "INNER JOIN SEG_USUARIO AS E3 ON E3.ID_USUARIO = E.ID_USUARIO "
			+ "INNER JOIN MED_RETORNO_LEITURA AS F3 ON F3.ID_LEITURA = A3.ID_LEITURA AND F3.ID_REGIONAL = ?1 AND F3.FL_ATIVO = 1 "
			+ "WHERE B3.ID_REGIONAL = ?1 AND B3.DT_ANO_MES_REF = ?2 AND C3.CD_GRUPO_FATURAMENTO = ?3 "
			+ "AND F3.ID_USUARIO = E.ID_USUARIO AND A3.CD_TAREFA_LEITURA = A.CD_TAREFA_LEITURA) "
			+ "AS DATAFIM, "
			...
			+ "AND RF.ID_USUARIO = E.ID_USUARIO "
			+ "AND A4.CD_TAREFA_LEITURA = A.CD_TAREFA_LEITURA) "
			+ "AS QTDFOTO "
			//+ "SUM(CASE WHEN F.ID_LEITURA IS NOT NULL AND F.ID_USUARIO = E.ID_USUARIO AND F.CD_TAREFA_LEITURA = A.CD_TAREFA_LEITURA THEN 1 ELSE 0 END) AS QTDFOTO "
			...
			+ "LEFT OUTER JOIN MED_RETORNO_LEITURA AS F ON F.ID_LEITURA = A.ID_LEITURA AND F.ID_REGIONAL = ?1 AND F.FL_ATIVO = 1 "
			...
			+ "C.CD_GRUPO_FATURAMENTO, E.ID_USUARIO, E.NM_NOME, B.DT_ANO_MES_REF, A.CD_TAREFA_LEITURA "
			+ "ORDER BY A.CD_TAREFA_LEITURA, E.ID_USUARIO ASC ", nativeQuery = true)
	...
	@Query(value = "SELECT "
			...
			+ "INNER JOIN MED_RETORNO_LEITURA AS D4 ON D4.ID_LEITURA = A4.ID_LEITURA AND D4.ID_REGIONAL = ?1 "
			...
			+ "LEFT OUTER JOIN MED_RETORNO_LEITURA AS D5 ON D5.ID_LEITURA = A5.ID_LEITURA AND D5.ID_REGIONAL = ?1 AND D5.FL_ATIVO = 1 "
