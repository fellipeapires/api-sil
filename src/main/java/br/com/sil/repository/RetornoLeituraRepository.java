...
public interface RetornoLeituraRepository extends JpaRepository<RetornoLeitura, Long>, RetornoLeituraRepositoryQuery {
	...
	@Query(value = "SELECT CONVERT(DATETIME, CONCAT(I.DT_PREVISAO_FIM, ' ', CONVERT(VARCHAR(8), B.DT_LEITURA, 114)), 20) AS DATALEITURA, "
			...
			+ "CASE WHEN LEN(B.CD_LATITUDE) < 7 AND B.CD_LATITUDE != '0' THEN REPLICATE(' ', 7- LEN(B.CD_LATITUDE)) ELSE B.CD_LATITUDE AS LATITUDE, "
			+ "CASE WHEN LEN(B.CD_LONGITUDE) < 7 AND B.CD_LONGITUDE != '0' THEN REPLICATE(' ', 7- LEN(B.CD_LONGITUDE)) ELSE B.CD_LONGITUDE AS LONGITUDE, "
